...
__all__ = [
    ...
    "CachingTorchDispatchMode",
    "selective_checkpointing_context_gen",
]
...
# (i.e. there could be CachedTorchDispatchMode calls that doesn't map to a CachingTorchDispatchMode call),
...
class CachingTorchDispatchMode(TorchDispatchMode):
    ...
    that's compatible with torch.compile. Used together with CachedTorchDispatchMode.
    """
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        if kwargs is None:
            kwargs = {}
        should_not_recompute = self.policy_fn(self, func, *args, **kwargs)
...
class CachedTorchDispatchMode(TorchDispatchMode):
    r"""
    A :class:`TorchDispatchMode` to implement selective activation checkpointing
    that's compatible with torch.compile. Used together with CachingTorchDispatchMode.
    """
    def __init__(self, policy_fn, storage):
        self.policy_fn = policy_fn
        self.storage = storage
...
def selective_checkpointing_context_gen(policy_fn):
    """
    A helper function to generate contexts for selective checkpointing.
    ...
    return CachingTorchDispatchMode(policy_fn, storage), CachedTorchDispatchMode(policy_fn, storage)
